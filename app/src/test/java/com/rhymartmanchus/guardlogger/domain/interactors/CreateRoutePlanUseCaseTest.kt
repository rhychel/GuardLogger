...
class CreateRoutePlanUseCaseTest {
    ...
    lateinit var useCase: CreateRoutePlanUseCase

    @Before
    fun setUp() {
        MockKAnnotations.init(this)

        useCase = CreateRoutePlanUseCase(gateway, shiftsGateway)
    }

    @Test
    fun `should be able to create a route plan for current session`() = runBlocking {
        ...
        val createCaptor = slot<RoutePlanRequests.Create>()
        coVerifySequence {
        ...
        assertEquals(1234, result.routePlanId)
    }
